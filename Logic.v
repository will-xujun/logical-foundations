...
Theorem plus_leb: forall n m q: nat, n + m =q -> (n <=?q) =true.
Proof. intros n m q. generalize dependent q. induction n as [|n' IH].
- reflexivity.
- simpl. destruct q as [|q'] eqn:E.
  + intros H. discriminate H.
  + intros H. injection H as H. specialize IH with (q:=q') as IH. apply IH in H. 
  apply H. Qed.

Lemma leb_O_equal_O: forall n : nat, n<=?0 =true -> n=0.
Proof. intros n. induction n as [|n' IH].
- intros. reflexivity.
- intros. simpl in H. discriminate H. Qed.

Example plus_is_O :
  forall n m : nat, n + m = 0 -> n = 0 /\ m = 0.
Proof. intros n m H. 
  apply conj.
  - Search Arith_base.le_n_0_eq_stt. assert (n <=? 0=true). 
    {apply plus_leb with (m:=m). apply H. } 
    apply leb_O_equal_O. apply H0.
  - Search Arith_base.le_n_0_eq_stt. assert (m <=? 0=true). 
    {  specialize add_comm with (m:=m) as comm.
    specialize comm with (n:=n) as comm. rewrite comm in H. 
    specialize plus_leb with (n:=m) as H1. specialize H1 with (m:=n) as H1.
    specialize H1 with (q:=0) as H1.
...
Proof.
  (* WORKED IN CLASS *)
  intros [|n']. (*same as intros n. destruct n as [|n'].*)
  - left. reflexivity.
  - right. reflexivity.
Qed.
...
Proof.
  ...
  - left. reflexivity.
  - right. specialize mult_S_O with (n:=n') as H1. specialize H1 with (m:=m) as H1.
...
Theorem not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P H Q. unfold not in H. intros HP. apply H in HP. destruct HP. Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H NOTQ. unfold not in NOTQ. unfold not. intros HP. 
  apply H in HP. apply NOTQ in HP. destruct HP. Qed. 
(** [] *)
...
Proof.
  ...
  destruct H as [H1 H2].
  apply H2 in H1. destruct H1. Qed.
(** [] *)
...
Theorem de_morgan_not_or : forall (P Q : Prop),
    ~ (P \/ Q) -> ~P /\ ~Q.
Proof.
  intros P Q H. unfold not. unfold not in H. 
  split.
  - intros HP. specialize or_intro_l with (B:=Q) as H1.
  apply H1 in HP. apply H in HP. destruct HP. 
  - specialize or_commut with (P:=Q) as H1.
    specialize H1 with (Q:=P) as H1.
    intros HQ. specialize or_intro_l with (B:=P) (A:=Q) as H2.
    apply H2 in HQ. apply H1 in HQ. apply H in HQ. destruct HQ. Qed.
(** [] *)
...
Proof.
  unfold not. intros H. specialize H with (n:=0) as H. simpl in H. discriminate H. Qed.
(** [] *)
...
Proof.
  intros X x xs. unfold not. intros H.
  assert (H1: disc_fn' X []). { simpl. apply I. }
...
Proof.
  ...
  - (* <- *)
    intros H. rewrite H. unfold "<>". intros H'. discriminate H'.
Qed.
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R HiffPQ HiffQR. split.
  - intros HP. apply HiffQR. apply HiffPQ. apply HP.
  - intros HR. apply HiffPQ. apply HiffQR. apply HR. Qed.
(** [] *)
...
Proof.
  intros P Q R. split.
  - intros H. destruct H as [HP| H2].
    + split. left. apply HP. left. apply HP.
    + destruct H2 as [HQ HR]. split. right. apply HQ. right. apply HR.
  - intros H. destruct H as [H1 H2]. destruct H1 as [HP|HQ].
    left. apply HP. destruct H2 as [H21 |H22].
    + left. apply H21. 
    + right. split. apply HQ. apply H22.
    Qed. 
(** [] *)
...
Proof.
  intros n m p. Check mul_eq_0.
  rewrite mul_eq_0. rewrite mul_eq_0. Check or_assoc. rewrite or_assoc.
  reflexivity.
Qed.
...
Proof.
  intros X P H. unfold not. intros H1. destruct H1 as [x E]. specialize H with (x:=x).
  apply E in H. destruct H. Qed. 
(** [] *)
...
Proof.
   ...
   - intros [x E]. destruct E as [H1|H2].
    + left. exists x. apply H1. + right. exists x. apply H2.
   - intros [G1| G2].
    + destruct G1 as [x0 G1]. exists x0. left. apply G1. 
    + destruct G2 as [x0 G2]. exists x0. right. apply G2. Qed.
(** [] *)

(** **** Exercise: 3 stars, standard, optional (leb_plus_exists) *)
Search "<=?".

Lemma leb_transitive : forall n m q, (n<=? m=true) /\ (m<=?q)=true -> (n<=?q)=true.
Proof. intros n m q [H1 H2]. generalize dependent m. generalize dependent q. induction n as [|n' IHn'].
  - simpl. reflexivity.
  - Search "<=?". intros q m H1 H2. specialize IHn' with (m:=pred m)(q:=pred q) as IHn'.
    destruct m as [|m'].
      + discriminate H1.
      + simpl in IHn'. simpl in H1. apply IHn' in H1 as IHn''.
        * Search "<=?".
          specialize plus_leb_compat_l with (n:=n') (m:=Nat.pred q) (p:=1) as thm.
          apply thm in IHn'' as IHn''. destruct q as [|q'].
          -- discriminate H2.
...
Lemma leb_Sm_m_false : forall m, S m <=? m =false.
Proof. intros m. induction m as [|m' IH].
- reflexivity.
- apply IH. Qed.
...
Proof.
  ...
  - destruct m as [|m'].
    ...
    + simpl in H.
    assert ((n' <=? S m') = true). {
      Search leb. specialize NatList.leb_n_Sn with (n:=m') as H1. 
      specialize leb_transitive with (n:=n') (m:=m') (q:=S m') as H2.
      apply H2. apply conj. apply H. apply H1.
    }
    apply IHn' in H0 as H1. destruct H1 as [x1 H2]. exists (pred x1). simpl.
    destruct x1 as [|x1'].
      -- rewrite add_comm in H2. simpl in H2. rewrite <- H2 in H.
        specialize leb_Sm_m_false with (m:= m') as contra.
        rewrite contra in H. discriminate H.
      -- simpl. rewrite add_comm in H2. simpl in H2. rewrite add_comm in H2. apply H2.
  Qed.


Theorem plus_exists_leb : forall n m, (exists x, m = n+x) -> n <=? m = true.
Proof. intros n m [x H]. generalize dependent m. induction n as [|n' IHn'].
  - simpl. reflexivity.
  - intros m0. specialize IHn' with (m:=pred m0) as IH.
    intros H. rewrite H in IH. simpl in IH. Search "=?".
   specialize n_equal_n_true with (n:=n'+x) as H1. Search "=?". apply eqb_true in H1.
   apply IH in H1. Search "<=?". apply plus_leb_compat_l with (p:=1) in H1.
   assert (1+(n'+x)=S n' +x) as H4. {
    simpl. reflexivity.
   } rewrite H4 in H1. rewrite <- H in H1.
   assert (1+n'=S n') as H5. {
    reflexivity.
   } rewrite <-H5. apply H1. Qed. 
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    ...
    + simpl. intros [H1 | H2].
      ...
      apply IHl' in H2. destruct H2 as [x1 H2'].
      exists x1. destruct H2' as [H21 H22].
      split. apply H21. right. apply H22.
  - intros [x [H1 H2]]. induction l as [|h t IH].
    + simpl in H2. destruct H2.
    + simpl in H2. destruct H2 as [H21 | H22].
      -- simpl. left. rewrite <- H21 in H1. apply H1.
      -- simpl. right. apply IH in H22. apply H22. Qed.
(** [] *)
...
Proof.
  ...
  - intros l' a. split.
    ...
    + intros [H1|H2]. simpl in H1. destruct H1. apply H2.
  - split. 
    + intros H. simpl in H. destruct H as [H1|H2].
      -- left. simpl. left. apply H1.
      -- apply IH' in H2 as [H3|H4]. left. simpl. right. apply H3.
           right. apply H4.
    + intros [H1|H2]. 
      -- simpl. simpl in H1. destruct H1 as [H2|H3].
        left. apply H2. right. simpl. apply IH'. left. apply H3.
      -- simpl. right. apply IH'. right. apply H2. Qed.
(** [] *)
...
Proof. intros T P l. split.
  - intros. induction l as [|h t IH].
    + simpl. apply I. 
    + simpl. simpl in H. specialize H with (x:=h) as H1.
      assert (h=h). { reflexivity. } apply or_intro_l with (B:=In h t) in H0.
      apply H1 in H0. split. apply H0.
      apply IH. intros x. specialize H with (x:=x).
      intros G. apply or_intro_l with (B:=h=x) in G. rewrite or_comm in G.
      apply H in G. apply G.
  - intros. induction l as [|h t IH].
    + simpl in H0. destruct H0.
    + simpl in H. simpl in H0. destruct H0 as [H1|H2].
      -- destruct H as [H3 H4]. rewrite <- H1. apply H3.
      -- apply IH. destruct H as [H3 H4]. apply H4. apply H2. Qed. 
...
Proof. intros Po Pev n H1 H2.
  destruct (even n) eqn:E. 
  - unfold combine_odd_even. destruct (even n) eqn:E1.
    Search negb.
    assert (odd n =false). { unfold odd. rewrite E1. reflexivity. }
    apply H2 in H. apply H. discriminate E.
  - unfold combine_odd_even. destruct (even n) eqn:E1.
    discriminate E.
    assert (odd n = true). { unfold odd. rewrite E1. reflexivity. }
    apply H1 in H. apply H. Qed.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros Po Pev n H1 H2. unfold combine_odd_even in H1.
  destruct (even n) eqn:E.
  - assert (odd n = false). { unfold odd. rewrite E. reflexivity. } 
...
Theorem combine_odd_even_elim_even :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Peven n.
Proof.
  intros Po Pev n H1 H2.
  unfold combine_odd_even in H1. destruct (even n) eqn:E.
  - apply H1.
  - assert (odd n =true). { unfold odd. rewrite E. reflexivity. }
    rewrite H in H2. discriminate H2.
    Qed.
...
Proof.
  intros x y z.
  rewrite add_comm. Check add_comm.
...
Lemma double_negb : forall b b': bool,
  b = b' <-> negb b = negb b'.
Proof. intros b b'. split.
  destruct b eqn:Eb.
  - destruct b' eqn:Eb'.
    + reflexivity.
    + intros H. discriminate H.
...
Proof.
  ...
  - destruct (even (S n')) eqn:E. 
    + rewrite (even_S n') in E.
      assert (even n' = false). { apply double_negb in E. Search negb. 
        rewrite negb_involutive in E. simpl in E. apply E. }
      rewrite H in IHn'. destruct IHn' as [k0 IHn0].
      exists (1+k0). simpl. rewrite <- IHn0. reflexivity.
    + Search even. rewrite even_S in E. apply double_negb in E.
      rewrite negb_involutive in E. simpl in E. rewrite E in IHn'.
      destruct IHn' as [k0 IH0].
      exists k0. rewrite IH0. reflexivity. Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (logical_connectives)
    ...
    in this chapter to the coasrresponding boolean operations. *)
...
Proof.
  intros b1 b2. split. 
  - intros H. split.
    + Search "&&". rewrite andb_commutative in H. Search andb. apply andb_true_elim2 in H.
      apply H.
...
Proof. intros []. reflexivity. reflexivity. Qed.

Lemma true_or_b : forall b, true || b= true.
Proof. intros []. reflexivity. reflexivity. Qed.

Lemma orb_comm: forall b c, b || c = c || b.
Proof. intros b c. destruct b. destruct c. reflexivity. reflexivity. destruct c. reflexivity. reflexivity. Qed. 
...
Proof.
  ...
  - intros H. destruct b1 eqn:E.
    + left. reflexivity. Search orb.
    + rewrite false_or_b in H. right. apply H. 
  - intros [H1|H2]. destruct b1 eqn:E.
    + reflexivity.
    + discriminate H1.
    + rewrite H2. Search orb. rewrite orb_comm. rewrite true_or_b. reflexivity.
  Qed.
(** [] *)
...
Proof.
  intros x y.
  split.
  - intros H. apply not_true_iff_false in H. unfold not. unfold not in H. intros H1.
    rewrite H1 in H. rewrite n_equal_n_true in H. apply H. reflexivity.
  - intros H. unfold not in H. apply not_true_iff_false. unfold not.
    intros G. apply eqb_eq in G. apply H in G. destruct G. Qed.
...
Proof. intros A eqb H. intros l1 l2. generalize dependent l2. induction l1 as [|h t IH].
  (* l1 is empty *)
  - intros [|h' t'].
  ...
  (* l1 is nonempty *)
  - intros [|h' t'].
    + split. intros H1. simpl in H1. discriminate H1. intros H1. discriminate H1.
    + split. 
      * intros H1. simpl in H1. apply andb_true_iff in H1 as [H2 H3].
        apply H in H2. apply (IH t') in H3. f_equal. apply H2. apply H3. 
      * intros H1. injection H1 as H2 H3. rewrite H2. rewrite H3. simpl.
        apply andb_true_iff. split.
        apply H. reflexivity. rewrite <- H3. apply (IH t). reflexivity.
        Qed.
...
Fixpoint forallb {X : Type} (test : X -> bool) (l : list X) : bool :=
  match l with
  | [] => true
  | h::t => if negb (test h) then false else forallb test t 
  end.
...
Proof. intros X test l.
  ...
  - intros H. induction l as [|h t IH].
    ...
    + simpl. simpl in H. destruct (test h) eqn:E.
      ++ split. reflexivity. simpl in H. apply IH in H. apply H. 
      ++ simpl in H. discriminate H. 
  - intros H. induction l as [|h t IH].
    + simpl. reflexivity.
    + simpl. simpl in H. destruct H as [H1 H2].
      rewrite H1. simpl. apply IH. apply H2. Qed.
...
Proof.
  ...
  apply not_both_true_and_false in H. destruct H. Qed.
...
Proof.
  intros ex X P H. Check excluded_middle. unfold not in H. 
  intros x. destruct (ex (P x)).
  - apply H0.
  - assert (exists x : X, ~ P x). { exists x. apply H0. }
    apply H in H1. destruct H1. Qed.
    
(* A Prop can be destructed if it is in the logical connective form, see before exercise
 with hypothesis of the form A\/B. *)
(** [] *)
