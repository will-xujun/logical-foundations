...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros X l. induction l as [|h t]. -reflexivity.
...
Proof.
  ...
  - simpl. rewrite IH. Search app. rewrite app_assoc. reflexivity. Qed. 

Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros X l. induction l as [|h t IH]. -reflexivity. 
  - simpl. Search rev. rewrite rev_app_distr. rewrite IH. reflexivity. Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (filter_even_gt7)
    ...
    7. *)
Notation "x >=? y" := (negb (leb x y)) (at level 70) : nat_scope.
Notation "x >? y" := ((negb (x =? y)) && (negb (x <=? y))) (at level 70) : nat_scope.
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => even x && (x >? 7)) l.
...
Theorem map_distr: forall (X Y : Type) (f: X -> Y) (l1 l2: list X),
  ...
  Proof. intros X Y f l1 l2. induction l1 as [|h t H]. -simpl. reflexivity.
...
- simpl. reflexivity.
- simpl. rewrite map_distr. rewrite IH. simpl. reflexivity. Qed.
...
Proof. intros X l. induction l as [|h t H].
- reflexivity.
- simpl. rewrite <- H. reflexivity. Qed.
...
(** Write down a theorem [fold_map_correct] stating that [fold_map] is
    ...
    [simpl].) *)
Theorem fold_map_is_map : forall (X Y:Type) (f:X->Y) (l: list X),
