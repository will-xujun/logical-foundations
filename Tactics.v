...
Theorem silly_ex : forall p,
  ...
  odd (S p) = true.
Proof.
  intros p eq1 eq2 H. apply eq2. apply eq1. apply H. Qed.
(** [] *)
...
Theorem rev_exercise1 : forall (l l' : list nat),
  ...
  l' = rev l.
Proof.
  intros l l' H. Search rev. symmetry. rewrite <- rev_involutive. rewrite <- H. reflexivity. Qed.
(** [] *)
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z l j eq1 eq2. injection eq1 as eqq1. rewrite eqq1.
  assert (H2: y :: l = z :: l). { transitivity j. apply H. apply eq2. }
  injection H2. intros H3. symmetry. apply H3. Qed.
(** [] *)
...
Proof.
  intros n. induction n as [|n' IHn'].
  - intros m. symmetry. apply eqb_0_l. apply H. 
  - intros m. destruct m as [|m'].
    + symmetry. discriminate H. 
    + intros H. specialize IHn' with (m:=m') as H1. 
    f_equal. simpl in H. apply H1 in H. apply H. Qed.
(** [] *)
...
Proof.
  ...
  - intros n H1. simpl in H1. destruct n as [|n'].
  + discriminate H1.
  + injection H1 as H2. specialize IHl' with (n:=n') as H3. apply H3 in H2. simpl. apply H2. Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2.
    ...
    -- destruct l2 as [|h2 t2] eqn:eq_l2.
      + simpl. intros H. discriminate H.
      + simpl. intros H. discriminate H.
  - intros l1 l2.
    destruct h as (x,y). simpl. destruct (split t). intros H.
    destruct l1 as [|h1 t1] eqn:eq_l1.
    -- destruct l2 as [|h2 t2] eqn:eq_l2.
      + discriminate H.
      + injection H as H1. discriminate H1.
    -- destruct l2 as [|h2 t2] eqn:eq_l2.
      + injection H as H1 H2 H3. discriminate H3. 
      + injection H as H1 H2 H3 H4. 
        specialize IHl with (l1:=t1).
        specialize IHl with (l2:=t2).
        simpl. f_equal. 
        ++ rewrite H1. rewrite H3. reflexivity.
        ++ apply IHl. rewrite H2. rewrite H4. reflexivity. Qed.
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros f b. destruct (f b) eqn:eq.
  - destruct (f true) eqn:E'. + apply E'. + destruct (f false) eqn:E''. 
    ++ reflexivity. ++ destruct b eqn:F. +++ rewrite E' in eq. discriminate eq.
...
Lemma eqb_0_r : forall (n : nat), (n=?0)=true -> n=0.
Proof. intros n. destruct n as [|n'].
- intros. reflexivity.
- intros H. simpl in H. discriminate H. Qed. 

Lemma eqb_S: forall (n m: nat),
  (n =? m) = (S n=?S m).
Proof. intros n m. generalize dependent m. induction n as [|n' IHn'].
  - intros m. destruct (0=?m) eqn:E.
    -- apply eqb_0_l in E. rewrite E. reflexivity.
    -- Search eqb. destruct m as [|m'].
      + discriminate E.
      + simpl. reflexivity.
  - intros m. destruct m as [|m'].
    -- simpl. reflexivity.
...
Proof.
  ...
  - intros m. Search eqb. destruct (0=? m) eqn:E.
    ++ apply eqb_0_l in E. rewrite E. reflexivity. 
    ++ symmetry. destruct m as [|m'].
      --- discriminate E.
      --- Search eqb. apply S_neqb_0.
  - intros m. destruct m as [|m'].
    ++ simpl. reflexivity.
    ++ specialize IHn' with (m:=m') as H. Search eqb. rewrite <- eqb_S. rewrite <- eqb_S.
       apply H. Qed.
(** [] *)
...
Lemma n_equal_n_true : forall (n: nat), (n=?n)=true.
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros n m p eq1 eq2. Search eqb. apply eqb_true in eq1. apply eqb_true in eq2. 
  rewrite eq2 in eq1. rewrite eq1. Search eqb. apply n_equal_n_true. Qed.
(** [] *)
...
Definition split_combine_statement : Prop := 
  ...
  forall X Y (l: list (X * Y)) l1 l2, length l1=length l2->
  combine l1 l2 =l -> split l = (l1, l2).

Theorem split_combine : split_combine_statement.
Proof. intros X Y l l1 l2 H0. generalize dependent l. generalize dependent l2. 
induction l1 as [|h1 t1 IH1].
  - intros l2 H1 l. destruct l2 as [|h2 t2].
    + simpl. intros H. rewrite <- H. reflexivity.
    + discriminate H1.
  - intros l2 H1 l. destruct l2 as [|h2 t2].
    + discriminate H1.
    + simpl. simpl in H1. specialize IH1 with (l2:=t2) as IH1. injection H1 as H1.
      destruct l as [|h t].
      -- intros contra. discriminate contra. 
      -- intros G1. specialize IH1 with (l:=t) as IH1. 
        injection G1 as G2 G3. apply IH1 in H1 as H2.
        * destruct h as (h1',h2'). simpl. rewrite H2.
          f_equal. f_equal. injection G2 as G21 G22. 
          ++ symmetry. apply G21.
          ++ f_equal. injection G2 as G21 G22. symmetry. apply G22.
        * apply G3.
Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X test x l lf. induction l as [|h t IHl].
  - simpl. intros H. discriminate H. 
...
Proof. intros X test l. generalize dependent test.
  induction l as [|h t IH].
  - intros test. simpl. unfold existsb'. simpl. reflexivity.
  - intros test. simpl. unfold existsb'. simpl. 
    destruct (test h) eqn:E.
    + simpl. reflexivity.
    + specialize IH with (test:=test) as H1. rewrite H1. unfold existsb'. simpl.
      reflexivity.
  Qed.
